...
void Player::CollisionMapCheckLeft(CollisionMapInfo& info) {
	...
	if (hit) {
		...
		info.move.x = std::min(0.0f, (rect.right - worldTransform_.translation_.x) + (0.9f + kBlank)); //\\
		// 壁に当たったことを記録する
		info.hitWall = true;
	}
}
...
void Player::CollisionMapCheckRight(CollisionMapInfo& info) {
	...
	if (hit) {
		...
		info.move.x = std::max(0.0f, (rect.left - worldTransform_.translation_.x) - (0.9f + kBlank)); //\\
		// 壁に当たったことを記録する
		info.hitWall = true;
	}
}
